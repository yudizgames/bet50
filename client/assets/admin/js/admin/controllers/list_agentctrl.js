 ...
 */
angular.module('main').controller('AgentListCtrl',function ($scope,$rootScope,$resource,$http,$state,toastr,DTOptionsBuilder,DTColumnDefBuilder,DTColumnBuilder,$compile) {
    console.log("Agent list controller call");
    ...
    $scope.userOperation = function(iUserId,OperationType){
        ...
        if(OperationType == 'view'){
            $state.go('admin.agentdetails',{'id':iUserId});
            ...
            $state.go('admin.agentform',{'id':iUserId,'action':'Edit'});
        }
    ...
    function listUser(){
        $scope.dtColumns = [
            ...
            DTColumnBuilder.newColumn("iUserId", "Agent ID").withOption('name', 'iUserId'),
            DTColumnBuilder.newColumn("vFullName", "Agent Name").withOption('name', 'vFullName'),
